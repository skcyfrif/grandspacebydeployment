document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch("http://88.222.241.45:9080/api/project/projects/dispatched/count")
    // fetch("http://localhost:9090/api/project/projects/dispatched/count")
        .then(response => {
            if (!response.ok) {
...
async function fetchProjects() {
    try {
        ...
        const response = await fetch('http://88.222.241.45:9080/api/project/dispatched-projects');
        // const response = await fetch('http://localhost:9090/api/project/dispatched-projects');
